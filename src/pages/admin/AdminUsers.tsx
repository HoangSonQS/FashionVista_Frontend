      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2 text-sm font-medium text-[var(--primary-foreground)] hover:bg-[var(--primary-hover)] transition-colors"
          >
                      ...
                      <tr key={user.id} className="hover:bg-[var(--muted)]/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            className={`rounded-lg border border-[var(--border)] bg-[var(--input-background)] px-2 py-1 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[var(--primary)] disabled:opacity-50 disabled:cursor-not-allowed ${user.role === 'ADMIN' ? 'bg-[var(--warning-bg)] text-[var(--warning)]' : 'bg-[var(--muted)] text-[var(--muted-foreground)]'
                              }`}
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${user.active
...

      {
        showCreateModal && (
          <AdminCreateUserModal
            ...
            onSuccess={() => {
              // Refresh data
              setData(null); // Force reload or manually call fetch
              adminUserService.getAllUsers(filters).then(setData);
            }}
          />
        )
      }
